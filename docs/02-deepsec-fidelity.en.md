...
16. **[VibeGuard deviation] Shape triage of low-entropy assignments (2026-09-18, found by dogfooding)**: DeepSec reports every non-placeholder `_sensitive_assignments` hit as `hardcoded_secret_assignment` critical. In practice that flooded the panel with `OPEN_TOKEN = "pay_corp:open"`, a regex assigned to `COUNTRY_TOKEN`, `…_CREDENTIAL_ID = "harness-dev-runtime"`, and `PASSWORD='strong-pass'` examples inside comments. `triageLowEntropyAssignment` in `l1-entropy.mjs` now sorts **low-entropy** values into three levels: value contains whitespace / regex-glob metacharacters / non-ASCII → **skip** (a pattern or prose); variable name ends in a name-like suffix (`_ID`, `_PREFIX`, `_NAME`, `…Hash`, …; deliberately not `key`) → **skip**; hit line is a comment, or the value has no digit → **low** (confidence 0.3, title marked "low risk"); everything else (digits, not a comment, entropy < 3.8) → critical as before. **The high-entropy path (`hardcoded_secret_high_entropy_assignment`) is untouched.** The old test pinning "whitespace value is still critical" now asserts the new behaviour.

17. **[VibeGuard deviation] Right-hand-side analysis for `sast_xss_inner_html` (2026-09-19, found by dogfooding)**: DeepSec's regex only recognises a leading `DOMPurify` / `sanitize` and only looks as far as the newline, so custom escapers (`esc()`), ternaries of pure literals (`open ? '&#9652;' : '&#9662;'`) and multi-line statements were all reported high; in one real file all 9 hits were false positives. The regex is kept verbatim (the quirk test still pins it); after a hit, `shield/l2-xss-triage.mjs` takes the whole statement (across lines, up to the semicolon) and analyses the right-hand side: every inserted value is a literal → **not reported**; every dynamic part is wrapped in a known escaping / sanitising / numeric function → **not reported**; the RHS is a call to a function defined in the same file, `.map(fn).join()` or a local variable → **the definition is looked up in the same file** and every `return` and assignment is analysed (recursion ≤ 3), counting as escaped only if all of them are safe; built by a generator whose definition cannot be found, or only partly verifiable → **medium** (confidence 0.4, title marked "to confirm", description says what to check); no escaping seen or unparseable → **high as before** (report rather than miss). This is not a JS parser: strings assembled with array `push()` / `join()` cannot be verified and land in medium. The original quirk (`innerHTML = DOMPurify.sanitize(x)` with a space still matches) remains at the regex level, but `scanSast` no longer reports it.
