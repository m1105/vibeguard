...
7. **No sensitive data in the tree** — code, comments, docs, tests, commit messages. No committed file may contain a string matching any of our secret rules: test fixtures are assembled at runtime with `fx('sk-ant-', '…')` (GitHub scans by shape and alerts even on obviously fake values); the only literal exceptions are `AKIAIOSFODNN7EXAMPLE` and a bare PEM header. Full policy in [`SECURITY.md`](SECURITY.md); enforced by `test/repo-hygiene.test.mjs`.
